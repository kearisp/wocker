...
export type LiquidEtherProps = PropsWithChildren<{
    mouseForce?: number;
    cursorSize?: number;
    isViscous?: boolean;
    viscous?: number;
...
export const LiquidEther: React.FC<LiquidEtherProps> = (props: LiquidEtherProps): React.ReactElement => {
    const {
        mouseForce = 20,
        cursorSize = 100,
        isViscous = false,
    ...
    const containerRef = useRef<HTMLDivElement>(null),
          ...
          webglRef = useRef<LiquidEtherWebGL | null>(null),
          resizeObserverRef = useRef<ResizeObserver | null>(null),
          rafRef = useRef<number | null>(null),
          intersectionObserverRef = useRef<IntersectionObserver | null>(null),
          isVisibleRef = useRef<boolean>(true),
    ...
    useEffect(() => {
        ...
        class AutoDriver {
            ...
            constructor(mouse: MouseClass, manager: WebGLManager, opts: {
                enabled: boolean; speed: number; resumeDelay: number; rampDuration: number
            }) {
                this.mouse = mouse;
...
`;
        ...
        class Viscous extends ShaderPass {
            ...
            update(...args: any[]) {
                const {viscous, iterations, dt} = (args[0] || {}) as {
                    viscous?: number; iterations?: number; dt?: number
                };
                if(!this.uniforms) return;
    ...
    return (
        <div ref={containerRef} style={style} className={`liquid-ether-container ${className || ''}`}>
